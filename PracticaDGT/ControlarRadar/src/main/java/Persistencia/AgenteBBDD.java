...
public ArrayList <Integer> get_idExp(){//nuevooooo
		...
		try{
			...
			resultado =  estado.executeQuery("SELECT * FROM Expediente");		
			
			
			while(resultado.next()){
				
				id.add(resultado.getInt("idExpediente"));
			...
			System.out.println("Error en entrar a la Tabla Expediente");
		}
...
public Expediente getExpediente(int id){//nuevoo
		Expediente exp=null;
		int rel;
		try{
			...
			resultado = estado.executeQuery("SELECT *  FROM expediente");
			while(resultado.next()){
				rel=resultado.getInt("idExpediente"); 
				if (id==rel){
					exp = new Expediente(resultado.getInt("idExpediente"),resultado.getInt("Matricula"),resultado.getString("DNI"),resultado.getString("Nombre"),resultado.getString("Apellidos"),resultado.getInt("idSancion"),resultado.getDouble("Precio"),resultado.getInt("puntos"));
					}
			...
			System.out.println("No se ha podido obtener un Expediente");
		}
		
		return exp;
		
	}
public void Insertar_Expediente(Expediente exp){//nuevo
	
try{
	
	
	
	ps= con.prepareStatement("INSERT INTO Expediente(idExpediente, matricula, DNI, Nombre, Apellidos, idsancion, precio, puntos) VALUES (?,?,?,?,?,?,?,?)");
	ps.setInt(1,exp.getIdExpediente());
	ps.setInt(2, exp.getMatricula());
	ps.setString(3, exp.getDni());
	ps.setString(4, exp.getNombre());
	ps.setString(5, exp.getApellidos());
	ps.setInt(6, exp.getId_sancion());
	ps.setDouble(7, exp.getPrecio_multa());
	ps.setInt(8, exp.getPuntos());
	...
	try{
		...
		System.out.println("No se ha podido obtener una verificacion");
	}
	return pagada;
